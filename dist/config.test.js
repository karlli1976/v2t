...
describe('resolveConfig', () => {
    ...
    it('config file values override defaults', async () => {
        ...
        vi.spyOn(fs, 'readFileSync').mockReturnValue(JSON.stringify({ language: 'en', model: 'base' }));
        const { resolveConfig } = await import('./config.js');
        const result = resolveConfig({});
    ...
    it('CLI flags override config file values', async () => {
        vi.spyOn(fs, 'existsSync').mockReturnValue(true);
        vi.spyOn(fs, 'readFileSync').mockReturnValue(JSON.stringify({ language: 'en' }));
        const { resolveConfig } = await import('./config.js');
    ...
    it('config file openaiApiKey overrides env var', async () => {
        ...
        vi.spyOn(fs, 'existsSync').mockReturnValue(true);
        vi.spyOn(fs, 'readFileSync').mockReturnValue(JSON.stringify({ openaiApiKey: 'from-config' }));
        const { resolveConfig } = await import('./config.js');
        const result = resolveConfig({});
